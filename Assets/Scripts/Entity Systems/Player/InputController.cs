...
namespace Entity_Systems.Player {
    public class InputController : PlayerInputControlsRevised.IPlayerActions, 
        PlayerInputControlsRevised.ILocomotionActions, 
        PlayerInputControlsRevised.IInventoryActions {
        #region Private Readonly Fields

        private InputStateHandler _inputStateHandler { get; set; }
        ...
        public void OnCrouch(InputAction.CallbackContext context) {
            _inputStateHandler.IsCrouch = !_inputStateHandler.IsCrouch;

            if (_inputStateHandler.IsCrouch) {
        ...
        public void OnRun(InputAction.CallbackContext context) {
            _locomotion.MaxSpeed = 4.0f;
            _animationBrain.Animate(out var state, AnimId.Run, 0.1f);
            Debug.Log("OnRun called.");
        ...
        public void OnInventoryTwo(InputAction.CallbackContext context) {
            var actions = new List<Action> {() => _animationBrain.Animate(AnimId.Idle), _animationBrain.StartIdleSequence};
            _animationBrain.Animate(AnimId.GrenadeThrow, 0.25f, actions);
        }
