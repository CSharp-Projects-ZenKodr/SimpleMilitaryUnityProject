...
namespace MonoManagers {
    public class MainCameraManager : MonoBehaviour{
        ...
        public bool allowForCameraPanning = false;
        [Range(2f, 25f)] 
        ...
        void Controller()
        {
            ...
            if (Input.mousePosition.y >= Screen.height - cameraBorder && allowForCameraPanning)
                position += new Vector3(TheCamera.transform.forward.x, 0, TheCamera.transform.forward.z) * (cameraSpeed * Time.deltaTime);
            if (Input.mousePosition.y <= 0 + cameraBorder && allowForCameraPanning)
                position -= new Vector3(TheCamera.transform.forward.x, 0, TheCamera.transform.forward.z) * (cameraSpeed * Time.deltaTime);
            if (Input.mousePosition.x >= Screen.width - cameraBorder && allowForCameraPanning)
                position += new Vector3(TheCamera.transform.right.x, 0, TheCamera.transform.right.z) * (cameraSpeed * Time.deltaTime);
            if (Input.mousePosition.x <= 0 + cameraBorder && allowForCameraPanning)
                position -= new Vector3(TheCamera.transform.right.x, 0, TheCamera.transform.right.z) * (cameraSpeed * Time.deltaTime);
            ...
            if (Input.GetKey(KeyCode.Mouse1))
            {
                if (_middleMouseInitial.x - Input.mousePosition.x > 100 || _middleMouseInitial.x - Input.mousePosition.x < -100)
                    rotation.y -= rotationFactor * (_middleMouseInitial.x - Input.mousePosition.x) / 60;
            ...
            if (Input.GetKey(KeyCode.Mouse2)) {
                if (_middleMouseInitial.y - Input.mousePosition.y > 100 ||
                    _middleMouseInitial.y - Input.mousePosition.y < -100)
                    position += transform.up * -(_middleMouseInitial.y - Input.mousePosition.y) / 480;
            }

            // Mouse Scroll Zoom
            float scroll = Input.GetAxis("Mouse ScrollWheel");
            position.y -= scroll * mouseScrollSpeed;
